...
void main() {
    ...
    // diffuse from the (now real) surface normal
    float diff = max(dot(N, sun_dir), 0.0);
    col *= (0.55 + 0.45 * diff) * (0.5 + 0.5 * sun_strength);

    // sharper specular glints as the storm builds
    vec3 H = normalize(sun_dir + V);
    float shininess = mix(60.0, 200.0, storm_intensity);
    float spec = pow(max(dot(N, H), 0.0), shininess);
    col += vec3(0.16, 0.17, 0.16) * spec * sun_strength;

    // hint of wind-blown foam on the steepest crests during storms
    float foam = smoothstep(0.8, 1.25, v_world.y / 1.2) * storm_intensity;
    foam *= 0.5 + 0.5 * n;   // break it up so it reads as flecks, not a sheet
    col = mix(col, vec3(0.55, 0.60, 0.62), foam * 0.25);

    // fresnel toward the haze
    float fres = pow(1.0 - max(dot(N, V), 0.0), 4.0);
    col = mix(col, fog_color, fres * 0.5);
