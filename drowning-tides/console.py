...
class Console:
    ...
    BAR_H = CONSOLE_HEIGHT * 2   # input line + a feedback line above it

    def __init__(self, app):
        self.app = app
        self.ctx = app.ctx
        self.program = app.shader_program.console
        self.font = pg.font.SysFont('menlo,monospace', CONSOLE_FONT_SIZE)
    ...
    def _rebuild_texture(self):
        ...
        pg.draw.rect(surf, CONSOLE_BG_COLOR, (0, CONSOLE_HEIGHT, self.W, CONSOLE_HEIGHT))
        line = CONSOLE_PROMPT + self.text + '_'
        ts = self.font.render(line, True, CONSOLE_TEXT_COLOR)
        surf.blit(ts, (8, CONSOLE_HEIGHT + (CONSOLE_HEIGHT - ts.get_height()) // 2))
        # feedback line
        if self.message:
            pg.draw.rect(surf, (8, 11, 14, 150), (0, 0, self.W, CONSOLE_HEIGHT))
            ms = self.font.render(self.message, True, (150, 165, 170))
            surf.blit(ms, (8, (CONSOLE_HEIGHT - ms.get_height()) // 2))
